---@meta

---@class ABP_Courtyard_Balcony_A_Double_Trappable_C : ABP_OMDTrapGrid_C
---@field Wood_Plank_600W_A_Light6 UStaticMeshComponent
---@field Wood_Plank_600W_A_Light5 UStaticMeshComponent
---@field Wood_Plank_600W_A_Light4 UStaticMeshComponent
---@field Wood_Support_A2 UStaticMeshComponent
---@field Wood_Support_A6 UStaticMeshComponent
---@field Wood_Support_A5 UStaticMeshComponent
---@field WoodPlank_4x1_Floor1 UStaticMeshComponent
---@field Wood_Plank_600W_A_Light3 UStaticMeshComponent
---@field Wood_Plank_600W_A_Light2 UStaticMeshComponent
---@field Wood_Plank_600W_A_Light1 UStaticMeshComponent
---@field Wood_Plank_600W_A_Light UStaticMeshComponent
---@field Wood_Support_A4 UStaticMeshComponent
---@field RSTTrapGridBox10 URSTTrapGridBoxComponent
---@field NavPowerExclusion UNavPowerExclusionComponent
---@field Wood_Support_A3 UStaticMeshComponent
---@field Wood_Support_A1 UStaticMeshComponent
---@field Wood_Support_A UStaticMeshComponent
---@field WoodPlank_4x1_Floor UStaticMeshComponent
ABP_Courtyard_Balcony_A_Double_Trappable_C = {}
